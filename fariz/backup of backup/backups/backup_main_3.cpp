...
#pragma endregion tutorial_triangles
	...
	for (int i = 0; i < maxParticles; i++) {
		smokes.push_back(Particle(glm::vec3(0, 0, -3.0f), glm::vec4(0.5f, 0, 0, 1), glm::vec3(0, 0, 0), 0, 1.0f));
	}
	GLfloat dt = 0;
	GLfloat scale = 0;
	while (!glfwWindowShouldClose(window)) {
		...
		if (dt < 2)
			...
			dt = 0;
		model = glm::translate(model, glm::vec3(0, 1, 0) * dt);
		model = glm::rotate(model, (float)glfwGetTime() * glm::radians(-55.0f), glm::vec3(1, 0, 0));
		model = glm::rotate(model, (float)glfwGetTime() * glm::radians(-55.0f), glm::vec3(0, 0, 1));
		scale = dt * 0.1;
		std::cout << 1 - dt << std::endl;
		smoke_shader.setFloat("opacity", 1 - dt);
		...
		glm::mat4 model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(smoke.scale));
		model = glm::translate(model, smoke.startPos);

		if (smoke.currentLife > 0.0f) {
		smoke.currentLife -= 0.05f * deltaTime;
		if (smoke.currentLife > 0.0f) {
		//smoke.velocity += glm::vec3(0, 1, 0) * (float)deltaTime;
		smoke.position += smoke.velocity * (float)deltaTime;
		}
		}
		else {
		smoke.position = smoke.startPos;
		smoke.currentLife = 5.0f;
		}

		model = glm::translate(model, smoke.position);
